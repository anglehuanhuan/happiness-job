...
const getWidthPercent = (data, titleWeigth = 10, contentWeight = 1) => {
	const widthLen = [];
	let count = 0;
	data.map((arr = [], index) => {
...
const MobileTable = ({ data = [[]], columnMinWidth = 69 }) => {
	...
	const widthPercent = useMemo(() => autoSize(getWidthPercent(data), columnMinWidth), []);
	return (
